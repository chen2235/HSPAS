...
- 在「交易紀錄管理（/trades）」頁面提供「上傳國泰證日對帳單」功能。
- 支援上傳「國泰證日對帳單.pdf」檔案，輸入固定密碼後解密並解析交易明細。
- 自動將 PDF 內的多筆交易明細映射到畫面上的「交易明細輸入列」，讓使用者檢視與確認，確認無誤後按「新增」一次性寫入多筆 `TradeRecord`。
- 交易明細中的「客戶應收付額」欄位，值為「+」代表賣出、值為「-」代表買入。

#### 4.5.2 前端畫面流程（/trades）

1. 在「交易紀錄管理」頁面上方或列表上方加入一個區塊「匯入國泰證日對帳單」：
   - 元件：
     - 檔案選擇按鈕（`<input type="file">`），僅接受 `.pdf` 副檔名。
     - 密碼輸入框（預設值顯示 placeholder：「請輸入對帳單密碼（預設：A120683373）」）。
     - 「解析對帳單」按鈕。
   - 限制：
     - 目前僅支援「國泰證券」格式之「日對帳單」PDF。
     - 檔案必須為可文字解析的 PDF（非純影像，或需後續 OCR 擴充）。

2. 使用者操作流程：
   - 使用者選擇檔案：`國泰證日對帳單.pdf`。
   - 在密碼欄位輸入密碼，預設規格為固定字串：`A120683373`。
   - 點擊「解析對帳單」按鈕後，前端呼叫後端 API 上傳 PDF 檔案與密碼。
   - 後端成功解析後，回傳一組「暫存交易明細清單」給前端，前端將此清單顯示在畫面上方的「待確認交易明細表格」。
...
3. 待確認交易明細表格：
   - 顯示欄位（依解析後的交易明細）：
     ...
     - 股票代號（對應 `StockId`）
     - 股票名稱（對應 `StockName`）
     - 交易別（買進／賣出；對應 `Action` = `BUY` / `SELL`）
     - 交易股數（對應 `Quantity`）
     - 成交單價（對應 `Price`）
     - 手續費（對應 `Fee`）
     - 交易稅（對應 `Tax`）
     - 其他費用（對應 `OtherCost`，如無則為 0）
     - 客戶應收付額（原始欄位，用於推算 `NetAmount` 與 Action）
   - 每一列右側可提供：
     - 單筆刪除按鈕（讓使用者排除不要匯入的明細）。
     - 可編輯欄位（例如備註 `Note`，或修正錯誤值）。

4. 確認與新增：
   - 在表格下方提供一個「新增」按鈕。
   - 使用者檢查所有交易列無誤後，點擊「新增」，前端將目前表格中的所有列資料以清單方式呼叫後端 API 建立多筆 `TradeRecord`。
   - 新增完成後：
     - 顯示成功訊息（例如「已成功新增 X 筆交易紀錄」）。
     - 清空暫存表格。
     - 重新載入下方既有的交易紀錄清單。

#### 4.5.3 交易方向與 NetAmount 判斷規則

- PDF 中的「客戶應收付額」欄位為判斷買賣方向與 `NetAmount` 的主要依據。
- 規則說明：
  - 若「客戶應收付額」為正值（前面帶「+」號或無號但定義為應收）：
    - 視為賣出交易（賣出股票）。
    - 對應到 `TradeRecord.Action = 'SELL'`。
    - 對應到 `TradeRecord.NetAmount` 為正值（實際金額）。
  - 若「客戶應收付額」為負值（前面帶「-」號）：
    - 視為買入交易（買入股票）。
    - 對應到 `TradeRecord.Action = 'BUY'`。
    - 對應到 `TradeRecord.NetAmount` 為負值（實際金額）。
  - 目前僅針對「股票／ETF 買賣」交易進行解析與匯入；其他非買賣性質款項（如股利入帳、利息、手續費調整等）如果出現在日對帳單中，一律忽略不產生 `TradeRecord`，後續如有需要再另行擴充規則。
- 備註文字說明（顯示在畫面上方）：
  - 「交易明細／客戶應收付額：+ 代表賣出金額（系統將記錄為 SELL，NetAmount 為正值）；- 代表買入金額（系統將記錄為 BUY，NetAmount 為負值）。」
##
## 4.5.4 後端 API 規格（草案）

1. 上傳與解析國泰證日對帳單
   - `POST /api/trades/cathay-daily-statement/parse`
   - Request（multipart/form-data）：
     - `file`: PDF 檔案（國泰證日對帳單.pdf）
     - `password`: string（PDF 開啟密碼，預設為 `A120683373`）
   - Response（200）：
     - 回傳解析後的交易明細陣列，例如：
       ```json
       [
         {
           ...
           "otherCost": 0,
           "netAmount": -580620,
           "customerReceivablePayableRaw": "-580,620"
         },
         {
           ...
           "otherCost": 0,
           "netAmount": 745000,
           "customerReceivablePayableRaw": "+745,000"
         }
       ]
       ```
     - `action` 與 `netAmount` 由後端依前述規則自動帶好。

2. 確認並新增多筆交易紀錄
   - `POST /api/trades/batch`
   - Request Body：
     - 內容為前端確認後的交易清單，欄位對應 `TradeRecord`：
       ```json
       {
         "items": [
           {
       ...
       ```
   - 後端逐筆建立 `TradeRecord`，並回傳成功筆數與失敗明細。

#### 4.5.5 錯誤處理與驗證

- 若 PDF 密碼錯誤或檔案非國泰證日對帳單格式：
  - 回傳適當錯誤訊息（例如「檔案無法開啟，請確認密碼是否為 A120683373」或「檔案格式不符合國泰證日對帳單」）。
- 解析出的每筆資料需進行基礎驗證：
  - `TradeDate` 必須為合法日期。
  - `StockId` 不為空字串。
  - `Quantity`、`Price` 大於 0。
  - `NetAmount` 與「客戶應收付額」方向一致（+ 為正、- 為負）。
- 若單筆資料驗證失敗：
  - 該筆可標記為錯誤，不加入「待確認交易明細表格」，或顯示紅色提示，讓使用者自行修正或刪除。

> 補充說明：本功能先專注於「日對帳單」中「現股買賣」資料，不處理融資融券、期權等其他商品，如後續有需求可再擴充解析規則與欄位對應。
